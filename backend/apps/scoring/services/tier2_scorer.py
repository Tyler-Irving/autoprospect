"""Tier 2 scorer — orchestrates deep-analysis Claude call, validation, and model persistence."""
...
class Tier2Scorer:
    """Score a single business with Claude at Tier 2 (deep-analysis) depth."""
    ...
    def score(self, business: Business) -> AutomationScore:
        ...
        prompt so Claude can build on existing analysis rather than starting
        from scratch.
        ...
        tier1_score = (
            business.scores.filter(tier="tier1").order_by("-scored_at").first()
        )
        ...
        logger.info(
            "Tier 2 scored business %d (%s): %d — cost %d¢",
            business.pk,
            business.name,
            score.overall_score,
    ...
    def _get_or_stub_enrichment(business: Business) -> EnrichmentProfile:
        """Return enrichment profile, creating a stub if none exists."""
        try:
    ...
    def _validate(data: dict[str, Any]) -> None:
        ...
        Args:
            data: Parsed JSON dict returned by ClaudeClient.complete().

        Raises:
            ValueError: On missing fields or out-of-range numeric values.
        """
        ...
        if deal not in VALID_DEAL_VALUES:
            # Coerce to low rather than failing the whole request
            data["estimated_deal_value"] = "low"

    @staticmethod
    def _save(business: Business, data: dict[str, Any]) -> AutomationScore:
        """Persist or update the Tier 2 AutomationScore.

        Args:
            business: Business instance the score belongs to.
            data: Validated and metadata-enriched dict from ClaudeClient.

        Returns:
            The created or updated AutomationScore instance.
        """
        score, _ = AutomationScore.objects.update_or_create(
            business=business,
            tier=AutomationScore.Tier.TIER2,
            defaults={
                ...
                "full_dossier": data.get("full_dossier", ""),
